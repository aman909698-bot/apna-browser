...
class TabManager {
  ...
  _handleNavigation(id, navUrl) {
    ...
    const blocked = blocker.isBlocked(navUrl);
    if (blocked) {
      blocker.recordBlocked();
      const blockedPath = `file://${path.join(__dirname, '..', 'src', 'blocked', 'blocked.html')}?url=${encodeURIComponent(navUrl)}&reason=${blocked}`;
  ...
  navigate(url) {
    ...
    const blocked = blocker.isBlocked(normalized);
    if (blocked) {
      blocker.recordBlocked();
      const blockedPath = `file://${path.join(__dirname, '..', 'src', 'blocked', 'blocked.html')}?url=${encodeURIComponent(url)}&reason=${blocked}`;
      tab.view.webContents.loadURL(blockedPath);
      return;
    }
